...
function displayRestaurant(){
	let dropdownMenu = document.getElementById("dropdown");
	index = dropdownMenu.selectedIndex;
...
function displayCategories(){
	...
	let menu = Object.keys(restaurants[index].menu);
	...
	displayItems(restaurants[index], menu);
	displayOrder();
}
...
function displayOrder(){
	...
	submit.addEventListener("click", function(){requestSubmitOrder(totalRounded, subtotalRounded, taxRounded);});
	...
	let minimumOrderRounded = Math.round((restaurants[index].min_order) * 100) / 100;
	...
	let deliveryPrice = restaurants[index].delivery_charge;
	totalRounded = Math.round((subtotalRounded + taxRounded + deliveryPrice) * 100) / 100;
...
//Once order has been submitted send information to server
function requestSubmitOrder(total, subtotal, tax){

	//Makes POST request
	let dropdownMenu = document.getElementById("dropdown");
	let index = dropdownMenu.selectedIndex;
	let added = {	
		restuarantID: restaurants[index].id,
		name: restaurants[index].name, 
		fee: restaurants[index].delivery_charge,
		order: order, 
		total: total,
		subtotal: subtotal,
		tax: tax
	...
	request.send(JSON.stringify(added));
}
...
function submitOrder(){
	...
	//Clears page and reloads restaurant info
	clearPage();
	displayCategories();
