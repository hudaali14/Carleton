...
app.post("/restaurants/:restID", updateRestaurant);
...
function getRestaurants(req, res, next){
    res.format({
        "text/html": function(){
            res.status(200).render("pages/restaurants", {restaurant: restaurants});
        },
...
function createRestaurant(req, res, next){
    res.format({
        "application/json": function(){
            ...
            restaurantNames.names.push({name: restaurants[restaurant.id].name, id: restaurants[restaurant.id].id});
...
function updateRestaurant(req, res, next){
    ...
    res.format({
        "application/json": function(){
            ...
            restaurantNames.names.push();
            restaurantIDs.restaurants.push();
            res.status(200).send();
        }
    });
}
