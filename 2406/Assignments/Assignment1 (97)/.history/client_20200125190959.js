...
function displayCategories(){
	...
	if(dropdownMenu.selectedIndex==1){
		...
		displayItems(aragorn, aragornMenu,index);
	}else if(dropdownMenu.selectedIndex==2){
		let legolasMenu=["Lembas", "Combos"];
		legolasMenu.forEach(element => {
			let para =document.createElement("p");
			para.innerHTML=element;
			//para.href="/Users/hudaali/Desktop/2406/assignment1#items"
			category.appendChild(para);
			index++;
		});
		displayItems(legolas, legolasMenu, index);
		...
		displayItems(frodo, frodoMenu, index);
	}else{
		let x=5;
	}
}

function displayItems(restaurant, restaurantMenu, index){
	let itemMenu = document.getElementById("items");
	//console.log(JSON.stringify(categoryName));
	let categoryItems = restaurant.menu[restaurantMenu[index]];
	// let name = legolas.menu["Lembas"][0].name;
	// console.log(name);
	for(let i=0; i<; i++){
		let item = document.createElement("p");
		item.innerHTML+="Name : ";
		item.innerHTML+=categoryItems[i].name;
