...
function init(){
	...
	category.addEventListener("mouseover", displayMenu(dropdownMenu.selectedIndex));
}
...
function displayMenu(index){
	if(index==0){
		...
		index=index+1;
		let categoryItems = document.getElementById("categories");
		let categoryList = restaurants.map(item => item.menu);
		console.log(JSON.stringify(categoryList[index].name));
		console.log("went through");
		categoryList.forEach(item =>{
			let para=document.createElement("p");
			para.value=item.value;
			categoryItems.appendChild(para);
		});
	}
