...
// Request restaurant names from server
function requestRestaurantNames(){
	...
	xhttp.onreadystatechange = function(){
		if(this.readyState == 4 && this.status == 200){
			...
			// Populates restaurant names to drop down menu
			let r = document.getElementById("dropdown");
...
//Requests restaurant info for displaying categories
function requestRestaurant(){
	...
	//Bases request on what menu should be present from the drop down menu
...
//Requests restaurant info for displaying the order information
...
function requestSubmitOrder(total){
	let xhttp = new XMLHttpRequest();
	let dropdownMenu = document.getElementById("dropdown");
	xhttp.onreadystatechange = function(){
		if(this.readyState == 4 && this.status == 200){
	...
	xhttp.send(JSON.stringify({name: dropdownMenu.value, order: order, total: total}));
}
...
function displayOrder(restaurant){
	...
	submit.addEventListener("click", function(){requestSubmitOrder(totalRounded);});
