...
// Calculates info for stats page
function calculate(){
	let item = orders[orders.length-1];
	let res = {};
	//Finds which restaurant data new order is meant to be added to
	statsinfo.forEach(element =>{
		if(item.name === element.name){
			res = element;
		}
	});
	//If option was found then update data
	if(res != {}){
		res.order++;
