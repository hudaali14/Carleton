...
STUDENT CLASS
PURPOSE: Student class with a student number, name, major, and gpa
COMPLEX OR CRITICAL MEMBERS:
    - Function operator<<(ostream&, Student&): Overloaded stream insertion operator
    and prints student info
    - Function operator<(Student&): Returns true if left-hand side student is 
    considered the lesser if its name comes first in alphabetical order
    - Function operator==(Student&): Returns true if their names and
